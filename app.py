...
with tab_heat:
    ...
    with hc1:
        ...
        st.caption(f"As of: **{asof_date.date()}**  |  window: last {heat_window} periods")
    ...
    if window_slice.notna().sum().min() < min_periods_needed or len(window_slice) < 3:
        ...
        mat = corr_full.copy()
        for i, b in enumerate(value_cols):
            for j, t in enumerate(value_cols):
                if j <= i:
                    mat.loc[b, t] = np.nan
        z = mat.values
        fig = go.Figure(
            data=go.Heatmap(
                z=z, x=value_cols, y=value_cols,
                colorscale="RdYlGn", zmin=-1, zmax=1,
                colorbar=dict(title="corr"),
                hovertemplate="Base: %{y}<br>Target: %{x}<br>Corr: %{z:.2f}<extra></extra>",
            )
        )
        fig.update_layout(height=650, xaxis_title="Target", yaxis_title="Base",
                           yaxis_autorange="reversed")
        apply_template(fig, dark_mode)
        with hc2:
            st.plotly_chart(fig, use_container_width=True, config=PLOTLY_CONFIG)
        st.caption("Use the Base/Target selectors near the top to drill into a pair "
                    "on the Time Series and Year Overlay tabs.")
